...
function FinishedPanel({ summary }: { summary: RoundSummary[] }) { return <section className="mx-auto max-w-2xl rounded-[2rem] border border-brand-200 bg-white p-8 text-center shadow-soft"><span className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-brand-100 text-3xl">✓</span><p className="mt-6 text-xs font-bold uppercase tracking-[0.2em] text-brand-700">Session complete</p><h1 className="mt-3 font-display text-4xl font-extrabold">Well spoken.</h1><p className="mt-3 text-sm text-slate-500">Both people completed a speaking round.</p>{summary.length > 0 && <div className="mt-7 space-y-2 text-left">{summary.map((round) => <div key={round.roundNo} className="rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-600"><strong className="mr-2 text-ink">Round {round.roundNo}</strong>{round.topicText}</div>)}</div>}<div className="mt-8 flex justify-center gap-3"><Link to="/history" className="secondary-button">View history</Link><Link to="/" className="primary-button">Home</Link></div></section>; }
...
function RoomLoading() { return <main className="min-h-screen bg-canvas px-5 py-6"><div className="mx-auto max-w-6xl"><Skeleton className="h-20 rounded-2xl" /><div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]"><div className="grid gap-4 sm:grid-cols-2">{[1, 2].map((seat) => <Skeleton key={seat} className="h-52 rounded-3xl" />)}</div><Skeleton className="h-72 rounded-3xl" /></div></div></main>; }

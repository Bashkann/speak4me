...
export function RoomPage() {
  ...
  return (
    <main className="min-h-screen bg-[#eff3ef] pb-28 md:pb-0">
      ...
      <AnimatePresence>{audio.playbackBlocked && <PlaybackBlockedBanner onResume={() => void audio.resumeAudio()} reducedMotion={Boolean(reducedMotion)} />}</AnimatePresence>
...
function PlaybackBlockedBanner({ onResume, reducedMotion }: { onResume: () => void; reducedMotion: boolean }) {
  return (
    <motion.button
      type="button"
      onClick={onResume}
      ...
      className="fixed inset-x-4 top-20 z-50 mx-auto flex max-w-md items-center justify-center gap-2 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-3 text-center text-sm font-bold text-amber-800 shadow-soft"
    >

          ...
          {!conversationId ? <div className="grid h-full place-items-center p-8"><EmptyState icon="💬" mood="encouraging" title="Choose a conversation" detail="Select a friend to continue practising together." /></div> : (
            <>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto bg-canvas px-4 py-5 sm:px-6">
                {messages.isLoading && <div className="space-y-3" aria-label="Loading messages"><Skeleton className="h-12 w-2/3 rounded-2xl" /><Skeleton className="ml-auto h-14 w-3/4 rounded-2xl" /><Skeleton className="h-10 w-1/2 rounded-2xl" /></div>}
                ...
                {messages.data && messages.data.items.length === 0 && <div className="grid h-full place-items-center"><EmptyState compact icon="✉" mood="encouraging" title="No messages yet" detail={`Say hello to ${selected?.partner.displayName ?? 'your friend'} to start the conversation.`} /></div>}

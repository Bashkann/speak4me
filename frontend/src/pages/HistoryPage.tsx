            ...
            {query.data.items.map((session, index) => (
              <article key={session.roomId} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:justify-end">{session.partners.map((partner) => <span key={partner} className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">{partner}</span>)}</div>
                </div>
